...
<h3>Choose a user for assignment '<?php echo $args[0]; ?>':</h3>
...
if (count($students) == 0) {
  echo '  <li>There are no students without grades for ' . $args[0] . '.</li>';
} else {
  $lab = false;
...
<h3>Enter a grade for '<?php echo $args[1]; ?>' on '<?php echo $args[0]; ?>':</h3>
...
?>
Grade (out of <?php echo $out_of[0]['Max_Score']; ?>): <input id="grade" type="text" />
